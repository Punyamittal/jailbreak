"""
Test security model on AI Agent Evasion Dataset.jsonl
"""
...
def load_ai_agent_evasion_dataset(file_path: Path):
    ...
    if not file_path.exists():
        ...
        return prompts, labels, attack_types
    ...
    try:
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            for line in f:
                ...
                try:
                    ...
                    attack_types.append(entry.get('attack_type', 'unknown'))
                    count += 1
        ...
        if labels:
            ...
            malicious_indices = [i for i, l in enumerate(labels) if l == 'jailbreak_attempt']
            if malicious_indices:
                malicious_attack_types = [attack_types[i] for i in malicious_indices]
                attack_type_counts = Counter(malicious_attack_types)
                print(f"    Attack type distribution (malicious prompts):")
                for attack_type, count in attack_type_counts.most_common(10):
...
def main():
    ...
    print("="*70)
    print("TESTING SECURITY MODEL ON AI AGENT EVASION DATASET")
    print("="*70)
    
    # Load test dataset
    print("\n[1/4] Loading test dataset...")
    
    prompts, labels, attack_types = load_ai_agent_evasion_dataset(Path("AI Agent Evasion Dataset.jsonl"))
    ...
    results = {
        'test_dataset': {
            'file': 'AI Agent Evasion Dataset.jsonl',
    ...
    results_file = Path("test_results_ai_agent_evasion.json")
